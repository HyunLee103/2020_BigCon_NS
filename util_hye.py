...
def preprocess_hye(train,test,drop_rate,k):
    #perform = del_outlier(train,drop_rate)
    #perform.reset_index(drop=True, inplace=True)

    # 방송 ID 부여 perform, test
    data = pd.concat([perform,test])
    data.reset_index(inplace=True,drop=True)
    data['show_id'] = make_sid(data)

    len_train = len(perform)

    perform = data.iloc[:len_train,:].reset_index(drop=True)
    test = data.iloc[len_train:,:].reset_index(drop=True)

    p_idsales = pd.DataFrame(perform.groupby('show_id')['취급액'].sum()).reset_index()
    p_idsales = km_clust(p_idsales, k, inner=0)

    sid_k = p_idsales['kmeans'].to_dict()

    # perform에 방송별로 구한 kmeans 값 부여
    perform['kmeans'] = perform['show_id'].map(lambda x: sid_k[x])
...
def mk_trainset_hye(data):
    ...
    data = data.drop(['방송일시','상품명','판매단가','length_raw','hour_gr','min_gr','len_gr','mcode_freq_gr','rating','gender','pay','set','special','show_order','show_norm_order','show_norm_order_gr'],axis=1)
