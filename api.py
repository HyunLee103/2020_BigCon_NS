...
class EXTERNAL_DATA:
    ...
    def __init__(self):
        ...
        self.client_id = "BU2RiBkSkYGYoL4AsKks"
        self.client_secret = "h9zoGYRyUg"
        self.perform_raw = pd.read_csv(r"data\2019_performance.csv")
        self.test = pd.read_csv(r"data\question.csv")

        self.perform_raw = self.perform_raw.apply(self.cleansing)
        self.perform_raw = self.perform_raw.apply(self.for_query)

        self.test = self.test.apply(self.cleansing)
        self.test = self.test.apply(self.for_query)

    # perform['방송일시'] 에다가 적용해서 사용하면 됨 / 새로운 col으로 만들어서 합칠 것
    def mk_datetime(self, col):
    ...
    def cleansing(self, data):
        ...
        data = re.sub('\[.*\]', '', data)
        data = re.sub('\[.*\］', '', data)
        for word in ['대', '통', '종', '세트', 'kg', 'g', '인용', '팩', '차', '미', '포', '구', 'L', '봉', '병', '매', '장', ',', '박스', '-', '개', '%']:
    ...
    def naver_query_trend_api(self):
        ...
        for query in self.perform['검색용']:
            ...
            if(rescode==200):
                ...
                json.loads(response_body.decode('utf-8'))['results'][0]['data']
                print(pd.DataFrame(response_body.decode('utf-8')['data']))
            else:
                print("Error Code:" + rescode)

train['상품군'].unique()
'패션의류', '패션잡화', '화장품/미용', '디지털/가전', '가구/인테리어', '출산/육아', '식품', '스포츠/레저', '생활건강', '여가/생활편의', '면세점'
50000000, 50000001, 50000002, 50000002, 50000002, 50000002, 50000002, 50000002, 50000002 
# 50000000 부터 10까지
...
body = "{\"startDate\":\"2017-08-01\", \
        ...
        \"category\":[{\"name\":\"패션의류\",\"param\":[\"50000000\"]},\
                    ...
                    {\"name\":\"생활건강\",\"param\":[\"50000008\"]}]    \
                    {\"name\":\"여가/생활편의\",\"param\":[\"50000009\"]}]    \
                    {\"name\":\"면세점\",\"param\":[\"50000010\"]}]    \
                        \}"
...
train['상품명전처리'] = train['상품명'].apply(cleansing)
train['검색명'] = train['상품명전처리'].apply(for_query)

train[['상품명', '상품명전처리', '검색명']].to_excel("test.xlsx")
